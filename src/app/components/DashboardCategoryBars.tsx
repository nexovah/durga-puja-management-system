...
// All-time category totals as 5 pillars: Chanda (paid/credited), Donation
// (category === 'donation'), Ads (category === 'ads'), Expenses (paid/
// credited), Loan (net received). Not tied to the collections-vs-expenses
// chart's date-range selector — these are running totals from the data
// already loaded into the other pages, no new query/DB change needed.
export function DashboardCategoryBars({ chandaList, donationAdsList, expenses, loansList }: DashboardCategoryBarsProps) {
  const { t } = useLanguage();
