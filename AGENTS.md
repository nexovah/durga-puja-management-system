...
- **Settings**: app configuration, incl. **Language** tab. User Management: per-page permissions (members/chanda/donationAds/expenses/treasury/settings) plus an **Access Level** (Can Edit & Manage / View Only) — view-only users see the pages their permissions allow but with no Add/Edit/Delete/Import controls (read-only tables, Export still works). Backed by `app_users.can_edit`; enforced client-side only (see security note in `supabase/003_view_only_access.sql`).
