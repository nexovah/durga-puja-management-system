...
- **Tasks** (`Tasks.tsx`): simple internal to-do list (Google Tasks/Notion-style) reached via the "⋮" more-menu, between Loans and Settings. Fields: Title*, Description, Priority* (High/Medium/Low/Note — 4-color badges: red/amber/blue/purple, each with a matching dot), `createdAt` captured automatically (read-only, always today on creation), **Assign To** (multi-select checkbox dropdown of saved Members — `Task.assignedMemberIds: string[]`), **Expiry Date** (defaults to `createdAt` + 15 days via `addDaysISO`, adjustable date picker; shown in red in the table once past today). Filterable by task name (search), priority, and created date; paginated like every table (`Pagination.tsx`). Gated by its own dedicated `tasks` permission (Settings → User Management checkbox, defaults on). Full add/edit/delete, logs to Activity Log under the `tasks` module.
