...
- **Dashboard**: overview of puja committee stats. Two-tier compact layout: horizontal "key figures" cards (Members, Chanda, Donation/Ads, Recent Chanda, **Pending/Due Chanda** [sum of pending chanda + unpaid remainder of partially-paid chanda; rejected excluded], Expenses total+count) at `grid-cols-2 sm:3 lg:4 xl:6`, plus smaller square icon-only "quick access" shortcuts below at `grid-cols-3 sm:4 lg:5 xl:8`. Deliberately mixed widget sizes/densities (not one uniform grid) to keep total height low on laptop (1024/1366/1600) and mobile widths.
