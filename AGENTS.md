...
- **CSV Import/Export**: Chanda Collection, Donation/Ads Collection, Expenses and Loans all have Export (existing) and **Import** (grey `#383737` button, white text) — bulk-add records from a `.csv` file, matching the export column layout. Shared parser/serializer: `src/app/lib/csv.ts`. Donation/Ads import matches category and Ads Category values against labels in any of the 3 languages so a CSV exported in one language re-imports correctly. Both buttons are hidden below the `sm` breakpoint (`hidden sm:flex`) — nobody imports/exports a CSV from a phone.
  - **Import Preview modal** (`ImportPreviewModal.tsx`): every CSV import opens this before anything is saved — shows how many rows were found in the file, how many are valid, and a line-by-line list of rejected rows with the reason. Nothing commits until the user clicks confirm; cancelling discards the parsed file entirely.
  - **Unique Bill/Voucher numbers** (`src/app/lib/uniqueCheck.ts` — `splitByDuplicateKey`, case-insensitive, blank = no check since the field is optional): Chanda's Bill Number, Donation/Ads' Voucher/Bill Number (Donation entries only), and Expenses' Voucher Number must each be unique. Enforced client-side on both the Add/Edit form (blocks save with an alert) and CSV import (duplicate rows — against existing data or against another row in the same file — are rejected in the Import Preview modal, not silently imported), and backstopped at the DB level by a partial unique index per field (`supabase/015_unique_bill_voucher_numbers.sql`). Loans has no unique field to check but still gets the same preview modal for a consistent import experience.
