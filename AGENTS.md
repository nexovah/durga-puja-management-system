...
- **Vendor** (`Vendors.tsx`): read-only report, not its own data — auto-collects every expense with a Vendor/Supplier Name filled in (Vendor/Supplier Name, Contact, Category, Date, Remarks), CSV export only. Gated by the `expenses` permission. Reached via the "⋮" (more) menu after Settings in the nav.
- **Loans** (`Loans.tsx`): full add/edit/delete/import/export page, same pattern as Chanda Collection. Fields: donor's name*, amount*, phone, **Payment Method** (Not Selected default / Cash / QR Scan / Online Banking / Check Payment), date*, **Return Date**, remarks. `paymentStatus` is always `'paid'` — not a user-facing field, no credit-logic branching needed. Gated by a dedicated `loans` permission. Reached via the "⋮" (more) menu after Settings.
